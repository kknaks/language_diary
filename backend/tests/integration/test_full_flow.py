...
# ---------------------------------------------------------------------------
# Full E2E flow (text messaging)
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_full_conversation_to_learning_flow(client, seed_user, tmp_path):
    """Full flow: WS connect → text messages → finish → diary → TTS → pronunciation."""
    ...
    resp = await client.get("/api/v1/diary/%d" % diary_id)
    assert resp.status_code == 200
    detail = resp.json()
    ...
    resp = await client.get("/api/v1/conversation/%s" % session_id)
    assert resp.status_code == 200
...
async def test_diary_complete_and_delete_flow(client, seed_user):
    ...
    # Mark complete
    resp = await client.post("/api/v1/diary/%d/complete" % diary_id)
    assert resp.status_code == 200
    assert resp.json()["status"] == "completed"
    ...
    # Soft delete
    resp = await client.delete("/api/v1/diary/%d" % diary_id)

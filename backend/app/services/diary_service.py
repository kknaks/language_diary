...
class DiaryService:
    ...
    async def get_list(
        ...
        limit = min(limit, 50)
        diaries = await self.repo.get_list(user_id, cursor=cursor, limit=limit)
    ...
    async def get_detail(self, diary_id: int, user_id: int) -> DiaryDetailResponse:
    ...
    async def delete(self, diary_id: int, user_id: int) -> None:
        diary = await self.repo.get_by_id(diary_id, user_id)
        if not diary:
            raise NotFoundError(
                code="DIARY_NOT_FOUND",
    ...
    async def complete(self, diary_id: int, user_id: int) -> DiaryResponse:
        diary = await self.repo.get_by_id(diary_id, user_id)
        if not diary:
            raise NotFoundError(
                code="DIARY_NOT_FOUND",
